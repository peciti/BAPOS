...
$(BUILD)/floppy.img: bootloader kernel
	...
	mcopy -i $(BUILD)/floppy.img apps/build/hello_world.bin "::hello.bin"
	mcopy -i $(BUILD)/floppy.img apps/build/snake.bin "::snake.bin"
...
$(BUILD)/snake_floppy.img:
	dd bs=512 count=2880 if=/dev/zero of=$(BUILD)/snake_floppy.img	
		...
		dd if=apps/build/snake.bin of=$(BUILD)/snake_floppy.img conv=notrunc
...
kernel:
	$(ASM) $(ASM_FLAGS) -o $(BUILD)/kernel/asm/asm_functions.obj $(SRC)/libraries/asm_functions/asm_functions.asm
	...
	$(CC16) $(CFLAGS16) -fo=$(BUILD)/kernel/c/fat12.obj $(SRC)/libraries/fat12/fat12.c
	$(LD16) NAME $(BUILD)/kernel.bin FILE \{$(BUILD)/kernel/asm/kernel.obj $(BUILD)/kernel/asm/power.obj $(BUILD)/kernel/asm/asm_functions.obj  $(BUILD)/kernel/asm/print.obj $(BUILD)/kernel/asm/disk.obj $(BUILD)/kernel/asm/input.obj $(BUILD)/kernel/c/kernel.obj $(BUILD)/kernel/c/stdio.obj $(BUILD)/kernel/c/fat12.obj \} OPTION MAP=$(BUILD)/kernel.map @$(KERNEL_SRC)/linker.lnk
